...
const initApp = () => {
  if (typeof WebsimSocket === 'undefined') {
    console.error('WebsimSocket not loaded. Retrying in 100ms...');
  ...
  class App {
    constructor() {
      ...
      console.log('Initial auth state:', {
        loggedIn: this.authManager.currentUser !== null,
    ...
    async init() {
      try {
        ...
        if (this.authManager.checkLoggedIn()) {
          ...
          console.log('No logged in user found, showing login page');
          this.showLoginPage();
        }
      } catch (error) {
    ...
    setupLoginSystem() {
      ...
      loginForm.addEventListener('submit', (e) => {
        ...
        console.log('Login attempt with:', username); // Debug logging
        
        if (this.authManager.login(username, password)) {
          console.log('Login successful');
          this.showMainContent();
          this.uiManager.showError('Login realizado com sucesso!');
          // Clear form
          loginForm.reset();
        } else {
    ...
    showLoginPage() {
      ...
      // Ensure the form is reset when showing login page
      document.getElementById('login-form').reset();
    }

    showMainContent() {
      console.log('Showing main content for user:', this.authManager.currentUser);
  ...
  // Initialize app when DOM is loaded and WebsimSocket is available
  window.app = new App();
};
