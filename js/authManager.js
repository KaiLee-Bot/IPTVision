export class AuthManager {
  ...
  setupDefaultAdmin() {
    ...
    const existingAdminIndex = this.users.findIndex(u => u.username === adminUser.username);
    if (existingAdminIndex >= 0) {
      this.users[existingAdminIndex] = adminUser;
    } else {
  ...
  logout() {
    ...
    localStorage.removeItem('global_playlist');
  }

  checkLoggedIn() {
    const savedUser = localStorage.getItem('iptvision_current_user');
    console.log('Checking saved user:', savedUser); // Debug logging
    ...
    if (savedUser) {
      try {
        ...
        console.log('Found existing user:', existingUser); // Debug logging
